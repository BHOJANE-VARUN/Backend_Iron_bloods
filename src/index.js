...
app.get("/getHospitalList",async (req,res)=>{
    try {
        const query = req.query.q || "";
        // console.log(query);
        const result = await Hospitals.find({ name: { $regex: query, $options: "i" } });
        res.json(result);
    } catch (e) {
      console.error(e);
      res.status(500).json({ error: "Internal Server" });
...
app.get("/getOneHospital", async (req,res) => {
  try {
    const query = req.query.q || "";
    // console.log(query);
    const result = await Hospitals.findOne({ name: { $regex: query, $options: "i" } });
    res.json(result);
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "Internal Server" });
